...
* There are two packages on the PCB at the same time, and AU or MU packages can be selectively used during production. You may receive one of the MU or AU packages. The difference between the two is only the package. The functions and performance are exactly the same. Please don't worry.
...
Click this link to get the [latest Arduino IDE](https://www.arduino.cc/en/Main/Software). 

There's Arduino Leonardo driver in the folder: Arduino/drivers.

After the IDE is installed, please select **Arduino Leonardo** at Tools > Board
...
!!!tip "The RX/TX led light up and never turn off"
    ...
    * Cehck if CANH and CANL is well connected
