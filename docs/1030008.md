...
CANBed V1 is a CAN Bus dev board. CANBed contains an Atmega32U4 microcontroller, which has **32KB** of Flash space and **2.5KB** of RAM space. The operating frequency is **16MHz**, which can meet most embedded applications. There's **Arduino Leonardo bootloader** inside the **Atmega32U4**. If you have used the Arduino IDE, programming will not be a problem for you. You can also use AVR Studio for development, we provide ICSP interface.
